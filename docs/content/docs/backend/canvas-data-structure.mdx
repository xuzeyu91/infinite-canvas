...
- 生成配置节点：`generationMode/model/imageModel/videoModel/textModel/audioModel/size/count/inputOrder` 保存生成配置；`generationMode` 可选择文本、图片、视频或音频；切换模式时会优先使用该模式自己的模型字段；上游输入通过 `connections` 计算。
