...
export function AppTopNav() {
    ...
    const activeToolSlug = navigationTools.some((tool) => !tool.external && tool.slug === slug) ? (slug as NavigationToolSlug) : undefined;
                    ...
                    <div className="mx-auto flex h-full max-w-7xl items-stretch justify-between gap-5 px-6">
                        <div className="flex min-w-0 items-center">
                            <Link href="/" className="flex h-full shrink-0 items-center gap-2 text-sm font-semibold leading-none tracking-tight text-stone-950 transition hover:text-stone-600 dark:text-stone-100 dark:hover:text-stone-300">
                                <img src="/logo.png" alt="BigBanana Canvas Logo" className="size-5 shrink-0 object-contain" />
                                <span className="text-base font-medium">BigBanana Canvas</span>
                            </Link>
                            ...
                            <nav className="hide-scrollbar ml-8 hidden h-16 min-w-0 items-center gap-7 overflow-x-auto md:flex">
                                {navigationTools.map((tool) => {
                                    ...
                                    if (tool.external) {
                                        return (
                                            <a key={tool.slug} href={tool.href} target="_blank" rel="noreferrer noopener" className={className}>
                                                {content}
                                            </a>
                                        );
                                    }
